// /partner/onboarding/error — client component per Next.js convention.
// Renders a token-styled fallback if a query throws (PostgREST outage,
// RLS policy changed, etc.). Reuses the existing AccountShell pattern.
...
export default function PartnerOnboardingError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && error.digest) {
      ...
      console.error('partner-onboarding route errored:', error.digest)
    }
  }, [error])
  ...
  return (
    <main className={styles.page} role="alert" aria-live="assertive">
      <div className={styles.card}>
        <h1 className={styles.h1}>We couldn&apos;t load the onboarding wizard.</h1>
        <p className={styles.lede}>
          Something went wrong on our side. Your previous answers are safe — try again, or come
          back in a minute.
        </p>
        {error.digest && (
