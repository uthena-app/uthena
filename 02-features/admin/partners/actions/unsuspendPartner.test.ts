// unsuspendPartner.test.ts — unit tests for the
// unsuspendPartnerAction server action.
...
import { unsuspendPartnerAction } from './unsuspendPartner'
...
describe('unsuspendPartnerAction', () => {
  ...
  it('returns invalid_input when the confirmation string is wrong', async () => {
    const res = await unsuspendPartnerAction({ id: 1, confirm: 'unsuspend' })
    expect(res.ok).toBe(false)
    if (!res.ok) expect(res.reason).toBe('invalid_input')
  })

  it('returns invalid_input for a non-positive id', async () => {
    const res = await unsuspendPartnerAction({ id: 0, confirm: 'UNSUSPEND' })
    expect(res.ok).toBe(false)
    if (!res.ok) expect(res.reason).toBe('invalid_input')
  })
  ...
  it('returns not_found when the partner row does not exist', async () => {
    ...
    const res = await unsuspendPartnerAction({ id: 99, confirm: 'UNSUSPEND' })
    expect(res.ok).toBe(false)
  ...
  it('returns not_suspended when the partner is already approved', async () => {
    ...
    expect(res.ok).toBe(false)
    if (!res.ok) {
      expect(res.reason).toBe('not_suspended')
      expect(res.error).toMatch(/approved/)
    }
  })

  it('returns not_suspended when the partner is still pending', async () => {
    mockFrom.mockImplementation(() =>
      makeChainable({ data: { id: 1, status: 'pending' }, error: null }),
    )
    const res = await unsuspendPartnerAction({ id: 1, confirm: 'UNSUSPEND' })
    expect(res.ok).toBe(false)
    if (!res.ok) expect(res.reason).toBe('not_suspended')
  })

  it('updates status to approved + writes audit row on the happy path', async () => {
    ...
    mockFrom.mockImplementation(() => {
      fromCallCount += 1
      if (fromCallCount === 1) {
        return makeChainable({ data: { id: 42, status: 'suspended' }, error: null })
      }
      if (fromCallCount === 2) {
        const chain: Record<string, unknown> = {}
        chain.eq = vi.fn(() => chain)
    ...
    const res = await unsuspendPartnerAction({ id: 42, confirm: 'UNSUSPEND' })
    ...
    // approved_at / approved_by must NOT be touched on unsuspend —
    // they survive the round-trip from the original approval.
    ...
    expect(auditPayload.action).toBe('admin.partner_unsuspended')
    ...
    expect(meta.before_status).toBe('suspended')
    expect(meta.after_status).toBe('approved')
  })

  it('rate limit shares the bucket with approve+suspend (20/hr/admin)', async () => {
    ;(requireAdmin as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 'unsuspend-rl-admin',
      email: 'unsuspend-rl@uthena.com',
      role: 'admin',
      display_name: 'UnRL',
    })
    mockFrom.mockImplementation(() =>
    ...
    const blocked = await unsuspendPartnerAction({ id: 1, confirm: 'UNSUSPEND' })
