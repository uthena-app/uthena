// page-rate-limit.ts — in-process per-user page rate limiter.
//
// P12.3 — used by `/partner/onboarding/welcome` and
// `/partner/onboarding/thanks` to satisfy the page-level rate-limit
// requirement in the spec acceptance criteria (60 req/min/user).
//
// Spec contract:
//   - `partner-onboarding-welcome.md` Security §line 71:
//     "Rate-limited to 60 req/min/user (page-level) to prevent scraper
//      abuse of the redirect logic".
//   - `partner-onboarding-thanks.md` Security §line 77:
//     "The page itself is rate-limited to 60 req/min/user via the
//      standard rate limiter in 00-foundations/auth/. Prevents a stuck
//      client from hammering the redirect logic."
//
// Why a separate module: Next.js `'use server'` files can only export
// async functions, and the page-level rate-limit is invoked from the
// RSC page loader (not a server action). The Map + the test reset
// helper are non-async + non-isolated to the call site, so they live
// here. Mirrors `saveStep.rate-limit.ts` (P12.2) and
// `exportLedgerCsv.rate-limit.ts` (P6.3).
//
