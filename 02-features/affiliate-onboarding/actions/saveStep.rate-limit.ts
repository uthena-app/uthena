// saveStep.rate-limit.ts — in-process rate limiter for the affiliate
// onboarding saveStep server action (P13.1).
//
// Spec contract: 60 requests/minute per user (same as the partner
// onboarding pattern — the spec doesn't pin a tighter limit because
// the wizard's expected cadence is "a few saves per minute at most").
//
// Why a separate module: Next.js `'use server'` files can only
// export async functions. The Map + the test reset helper are
// non-async + non-isolated to the action, so they live here. Matches
// the same pattern as `partner-onboarding/actions/saveStep.rate-limit.ts`
// (P12.2) and `payouts/actions/exportLedgerCsv.rate-limit.ts` (P6.3).
//
